...
pub async fn dump_store(
    ...
    match node_repository.root_hash().await {
        Ok(Some(root)) => {
            out.push_str(&format!("root: {}\n\n", root));
            let root = root.clone();
            dump_tree(node_repository, blob_repository, &root, ".", 0, &mut out).await;
        }
...
fn dump_tree<'a>(
    node_repository: &'a (impl NodeRepository + Send + Sync),
    blob_repository: &'a (impl BlobRepository + Send + Sync),
    hash: &'a Hash,
    name: &'a str,
    depth: usize,
    out: &'a mut String,
    ...
    Box::pin(async move {
        ...
        match node_repository.get_node(hash).await {
            ...
            Ok(Some(Node::File { blobs })) => {
                ...
                for blob_ref in blobs {
                    let missing = match blob_repository.contains(&blob_ref.hash).await {
                        Ok(true) => "",
