...
export const TrayPopup: React.FC = () => {
  ...
  const { connectionState, startDaemon, isStartingDaemon } = useDaemonStatus();
  ...
  return (
    ...
    >
      ...
      {connectionState === 'unreachable' ? (
        /* Daemon Unreachable */
        <div
          className="mx-4 mb-4 mt-1 bg-card border border-border rounded-xl px-5 py-[18px] shadow-card flex flex-col gap-3"
          data-testid="tray-daemon-unreachable"
        >
          <div className="flex flex-col gap-0.5">
            <div className="font-serif text-[1.2rem] font-bold text-foreground">
              Daemon not running
            </div>
            <div className="text-[0.76rem] text-muted-foreground">
              Phaneros can&rsquo;t reach the sync daemon. Start it to resume syncing.
            </div>
          </div>
          <button
            type="button"
            ...
            className="w-full h-9 rounded-lg bg-primary text-primary-foreground text-[0.82rem] font-semibold shadow-card hover:-translate-y-px transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            data-testid="tray-start-daemon-button"
          >
        ...
        <>
          ...
          <div className="mt-4" data-testid="tray-activity-stream">
            <div className="text-[0.72rem] font-bold uppercase tracking-wider text-muted-foreground px-5 pb-2">
              <span>Recent Syncs</span>
            </div>
            <div className="flex flex-col px-4 gap-1.5">
              {activity.length === 0 ? (
                <div
                  className="text-[0.76rem] text-muted-foreground px-1 py-2"
                  data-testid="tray-activity-empty"
                >
                  No recent syncs
                </div>
              ) : (
                activity.map((session) => (
                  <div
                    ...
                    data-testid={`tray-activity-item-${session.id}`}
                  >
                    ...
                    <span className="text-[0.7rem] text-muted-foreground flex-shrink-0">{session.timestamp}</span>
                  </div>
                ))
              )}
            </div>
