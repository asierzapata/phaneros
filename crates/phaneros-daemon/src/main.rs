...
///
/// Owns one sync engine per enabled drive and exposes a JSON-RPC control
/// plane over a unix socket for clients (the `phaneros` CLI, or a future
/// GUI) to observe and control them.
#[derive(Parser)]
